...
const Login: FC<{
  ...
  const { mutate, isLoading } = useMutation({
    ...
    mutationFn: (data: IUserPostAction) =>
      axiosInstance.post("/api/auth/login", data).then((res) => res.data),
    onSuccess: (data) => {
      setUser(data);
        ...
        <div className="mt-12 flex flex-col items-center">
          ...
          <div className="w-full flex-1">
            <div className="mx-auto max-w-md">
              ...
              />
              <p
                onClick={() => handleActionType("forgot-password")}
                className="text-sm text-sky-500 text-right mt-2 cursor-pointer hover:underline"
              >
                Forgot password
              </p>
